...
contract E2ETestDeploy is Script, IICS07TendermintMsgs {
    ...
    function run() public returns (string memory) {
        ...
        if (keccak256(bytes(verifierEnv)) == keccak256(bytes("mock"))) {
            ...
            console.log("=== Deployed mock verifier");
        ...
        // Add counterparty client
        bytes[] memory merklePrefix = new bytes[](2); // Create a dynamic array of size 2
        merklePrefix[0] = bytes("ibc");
        merklePrefix[1] = bytes("");
        IICS02ClientMsgs.CounterpartyInfo memory counterpartyInfo = IICS02ClientMsgs.CounterpartyInfo(
        string("08-wasm-0"), merklePrefix
        );
        string memory clientId = "cosmoshub-1";
        ics26Router.addClient(clientId, counterpartyInfo, address(ics07Tendermint));

        // Deploy a test ERC20
        TestERC20 erc20 = new TestERC20();

        // Mint some tokens
        erc20.mint(e2eFaucet, uint256(10 ** 24));
        ...
        vm.writeJson(finalJson, string.concat(root, "/scripts/addresses.json"));
